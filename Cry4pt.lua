...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "Cry4pt is already loaded... use 'killscript' to kill", nil),
...
do
    ...
    MetaMethodHooks.Namecall = function(...)
        ...
        -- ik this is horrible but Cry4pt v3 has a better way of doing hooks
...
AddCommand("commandcount", {"cc"}, "shows you how many commands there is in Cry4pt", {}, function(Caller)
...
AddCommand("advertise", {}, "advertises the script", {}, function()
    local ChatRemote = Services.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
    ChatRemote.FireServer(ChatRemote, "I am using Cry4pt, join the server 5epGRYR", "All");
end)

AddCommand("joinserver", {"discord"}, "joins the Cry4pt discord server", {}, function()
    ...
    if (Request({
        ...
        return "joined Cry4pt discord server"
    else
...
AddCommand("changelogs", {"cl"}, "shows you the updates on Cry4pt", {}, function()
...
AddCommand("commands", {"cmds"}, "shows you all the commands listed in Cry4pt", {}, function()
...
AddCommand("config", {"conf"}, "shows Cry4pt config", {}, function(Caller, Args, CEnv)
...
AddCommand("console", {"errors", "warns", "outputs"}, "shows the outputs Cry4pt has made", {}, function()
    local MessageClone = Clone(Console.Frame.List);
...
AddConnection(CConnect(ChatLogs.Save.MouseButton1Click, function()
    ...
    local String =  format("Cry4pt Chatlogs for %s (%s)\n\n", GameName, os.date());
...
AddConnection(CConnect(HttpLogs.Save.MouseButton1Click, function()
    local Children = GetChildren(HttpLogs.Frame.List);
    local Logs =  format("Cry4pt HttpLogs for %s\n\n", os.date());
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
            Logs = format("%s%s\n", Logs, v.Text);
...
AddConnection(CConnect(Console.Save.MouseButton1Click, function()
    ...
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Children = GetChildren(Console.Frame.List);
    local String =  format("Cry4pt logs %s\nGame: %s - %d\n\n", TimeSaved, GameName, game.PlaceId);
