-- Actor check and loadstring execution
...
if run_on_actor or run_on_thread then
    (run_on_actor or run_on_thread)(ac, [[
        ...
        local VisibilityCheck = getgenv().VisibilityCheck ~= false -- Default to true if not set
        ...
        getgenv().con = game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local ignoreModels = {clientCharacter}
            if VisibilityCheck then
                for _, modelName in {"glass", "boundary"} do
                    local model = workspace:FindFirstChild(modelName)
                    if model then
                        table.insert(ignoreModels, model)
                    end
                end
            end
            local raycastParams = RaycastParams.new()
            raycastParams.FilterType = Enum.RaycastFilterType.Blacklist
            raycastParams.FilterDescendantsInstances = ignoreModels
                ...
                if plr ~= Player then
                    ...
                    if health > 0 and character and (not TeamCheck or team ~= clientTeam) then
                        ...
                        if best_hitbox and min_distance < fov then
                            local bone = character:FindFirstChild(best_hitbox)
                            if bone then
                                local isVisible = true
                                if VisibilityCheck then
                                    local rayOrigin = Camera.CFrame.p
                                    local rayDirection = (bone.Position - rayOrigin)
                                    local raycastResult = workspace:Raycast(rayOrigin, rayDirection, raycastParams)
                                    isVisible = raycastResult and raycastResult.Instance:IsDescendantOf(character)
                                end
                                if isVisible then
